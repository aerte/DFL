...
def split_dataset_to_workers(num_workers, split):
    ...
    if split == "by_class":
        ...
        for i in range(num_workers):
            tr_im_clients["worker_%02d" % i] = np.reshape(tr_im[tr_la_shuffle_index[i]],
                                                          [len(tr_la_shuffle_index[i]), 28, 28, 1])
...
class VGG16(nn.Module):
    ...
    # VGG16 (D): https://arxiv.org/pdf/1409.1556.pdf
    def __init__(self, num_channel):
...
def create_cifar10_dir(conf):
    model_mom = "../exp_data/"
    model_dir = model_mom + "/cifar10/version_2/"
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)

    conf.folder_name = "cifar10"
    conf.dir_name = "version_2"
    return conf
