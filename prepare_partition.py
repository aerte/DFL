...
class DataSampler(object):
    ...
    def sample_indices(self):
        if self.data_scheme == "random_sampling":
            self.sampled_indices = self.conf.random_state.choice(self.indices,
    ...
    def use_indices(self, sampled_indices=None):
        ...
        return Partition(self.data,
                         indices=indices_to_use)  # data is the entire dataset, but this indices to use is a subset
...
def build_non_iid_by_dirichlet(random_state, indices2targets, non_iid_alpha, num_classes, num_indices, n_workers):
    ...
    split_n_workers = [n_auxi_worker if idx < num_split - 1 else n_workers - n_auxi_worker * (num_split - 1) for idx in
                       range(num_split)]
    ...
    def _create_indices(self, indices):
        if self.partition_type == "original":
            ...
            indices2targets = np.array(
                [(idx, s_target) for idx, s_target in enumerate(self.data.targets) if idx in indices])
            ...
            if not os.path.isfile(self.path2save_index):
                ...
                with open("../exp_data/%s/%s/" % (self.folder_name, self.conf.dir_name) + "sampling_index.obj",
                          "wb") as f:
