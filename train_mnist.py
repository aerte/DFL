...
def train_with_conf(conf):
    ...
    if conf.split != "iid" and conf.shuffle_percentage != 0:
        tr_im_sync, tr_la_sync, combine_im, combine_la = mnist_utils.shuffle_dataset(conf.n_clients,
                                                                                     conf.shuffle_percentage,
                                                                                     )
    else:
        combine_im, combine_la = [tr_im["worker_%02d" % i] for i in range(10)], [tr_la["worker_%02d" % i] for i in
                                                                                 range(10)]
